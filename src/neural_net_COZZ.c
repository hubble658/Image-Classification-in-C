...
void updateWeightsNeuronOld(Neuron* neuron, Layer* prevLayer , double learningRate, int batchSize) {
    int i;
    // printf("-----------\n");
    for (i = 0; i < prevLayer->neuronNum; i++)
    {
        ...
        //Gradient clipping
        double avgGradient = prevNeuron->delta_weights[0] / batchSize;
        // printf("%lf\n",(prevNeuron->m_output_val) * neuron->m_gradient);
        // printf("%lf \n",val);
        if(avgGradient >  GRAD_CLIP) avgGradient =  GRAD_CLIP;
        if(avgGradient < -GRAD_CLIP) avgGradient = -GRAD_CLIP;
        if((prevNeuron->m_output_val) * neuron->m_gradient != avgGradient){
            // printf("AAAAAAAAAA %lf - avg : %lf\n",(prevNeuron->m_output_val) * neuron->m_gradient,avgGradient);
        }
...
void updateWeightsNeuron(Neuron* neuron, Layer* nextLayer , double learningRate, int batchSize) {
    ...
    for (i = 0; i < nextLayer->neuronNum ; i++)
    {
        ...
        // double delta = neuron->delta_weights[nextNeuron->m_myIndex] / batchSize;
        //Gradient clipping
        // if(avgGradient >  GRAD_CLIP) avgGradient =  GRAD_CLIP;
        // if(avgGradient < -GRAD_CLIP) avgGradient = -GRAD_CLIP;
        neuron->weights[nextNeuron->m_myIndex] += learningRate * neuron->delta_weights[nextNeuron->m_myIndex]/batchSize;
        // neuron->weights[nextNeuron->m_myIndex] += learningRate * (neuron->m_output_val) * nextNeuron->m_gradient;
    }
    // printf("SUm : %lf\n",neuron->m_delta_sum);
}
...
void resetGradientsSum(Net* net) {
    ...
    for (i = 0; i < net->layerNum - 1; i++)
    {
        for (j = 0; j < net->layers[i]->neuronNum; j++)
        {
            ...
            {
                net->layers[i]->neurons[k]->delta_weights[k] = 0;
            }
            //Weight Clipping
            for (k = 0; k < net->layers[i]->neurons[j]->weightCount; k++)
            {
...
//     neuron->m_delta_sum += neuron->m_gradient;
...
//         // outputLayer->neurons[i]->m_delta_sum += delta * derivActivation(outputLayer->neurons[i]->m_output_val);
//     }
// }
        ...
        for (int n = 0; n < currentLayer->neuronNum - 1; ++n) {
            ...
            for (int j = 0; j < nextLayer->neuronNum - 1; j++) {
                dow += currentNeuron->weights[j] * nextLayer->neurons[j]->m_gradient;
                 currentNeuron->delta_weights[j] += currentNeuron->m_output_val *  nextLayer->neurons[j]->m_gradient;
            }
